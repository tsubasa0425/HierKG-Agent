...
2. **只做检索不做生成** —— KGRetrieve 没有任何 LLM 调用，答案合成由外层 Agent / LLM 负责
3. **统一元数据** —— 每个工具有 `name` / `description` / `parameters`（JSON Schema）/ `output_schema`
4. **零数据库依赖** —— 默认纯内存图（KGMemory 加载 `final_kg.json`），后续可替换为数据库后端
...
| 后端 | 类名 | 依赖 | 适用场景 |
|------|------|------|---------|
| 纯内存 | `KGMemory` | 零依赖 | 快速原型、小规模数据（< 5000 节点） |
| 数据库 | `KGDBMemory` | Neo4j + ChromaDB + Ollama | 持久化、跨进程共享、语义搜索 |

两种后端**接口完全一致**，`ToolRegistry` 和 14 个工具零改动。
...
# —— 方式 A：内存后端（零依赖）——
kg = KGMemory.load("src/HiddenKG/output/final_kg.json")
reg = ToolRegistry(kg)

# —— 方式 B：数据库后端（Neo4j + ChromaDB）——
from src.KGRetrieve import KGDBMemory
kg = KGDBMemory()                        # 自动读取 storage.yaml 配置
reg = ToolRegistry(kg)                    # 工具层完全一致

# 首次使用需要先导入数据（只需一次）：
# python -m src.KGRetrieve.db_backend import

# —— 通用调用 ——
schemas = reg.get_tool_schemas()          # 14 条 OpenAI function schema
...
| **模糊搜索** | 4 | `search_concepts` / `search_entities` / `search_evidences` / `semantic_search` | 不知道 ID，按关键词探索 |
| **二次加工** | 1 | `rank_results` | 对前几步结果重排序 / 手动挑选 |
...
> 模糊搜索支持两种后端：
> - **内存后端（KGMemory）**：基于名称/别名/描述的字符串匹配，零依赖
> - **数据库后端（KGDBMemory）**：ChromaDB 向量语义搜索 + Neo4j 名称精确匹配混合，语义相关性更强
>
> 返回的每条结果都带 `match_score` 字段，分数越高越相关。
...
对前几步检索返回的候选节点列表做二次重排序。

**两种模式**：
- **打分重排模式**（推荐）：传入 `node_ids` + `intent_query`，工具对每个节点按意图关键词在 name/description/aliases/snippet 中的命中数打分，按分数降序返回 top_n
...
| `intent_query` | string | 否 | — | 排序意图（如 `找智能体的先修概念`）。用于对每个节点做关键词打分 |
...
```jsonc
{
  ...
  "items": [
    {"node_id": "concept_ffee9ff0", "name": "大语言模型驱动的智能体",
     "rank_score": 2.0, "related_evidence_ids": ["ev_xxx"]}
  ]
}
```

**使用建议**：
- 当前面的搜索工具返回太多结果（如 `search_concepts` 返回 15 条），而你只想要最相关的 3-5 条时，用打分重排模式
- 当 Agent 已经自己挑好了一些 ID（基于 LLM 的判断），只想按原顺序取前 N 条时，用手动挑选模式
- 工具不依赖 LLM，打分是纯关键词命中数，零成本零延迟
...
完整代码见 [demo_agent_orchestration.py](file:///d:/workspace/KG-for-structure-file/src/KGRetrieve/demo_agent_orchestration.py)。
...
2. **为什么有 `rank_results` 但没有 `rerank_with_llm`？** 工具集严格「零 LLM 依赖」。如果需要 LLM 重排，Agent 自己用 LLM 把 `node_ids` 的描述过一遍就行，不必做成工具。
3. **为什么有 `semantic_search` 还要分 `search_concepts/entities/evidences`？** `semantic_search` 是「探索型第一步」，而单层搜索用于「明确知道要查哪层」的精确场景，返回结果更聚焦。
4. **为什么支持双后端？** `KGMemory`（内存）零依赖、快速启动，适合原型开发和小规模数据；`KGDBMemory`（Neo4j+ChromaDB）支持持久化、跨进程共享、向量语义搜索，适合生产部署。两者接口完全一致，`ToolRegistry` 和 14 个工具零改动。
5. **旧版 `AgenticRAG` 已删除**：旧版硬编码 `classifier → selector → executor → generator` 路由流水线整体弃用，当前唯一检索路径是 KGRetrieve 14 个原子工具 + Agent 自主调用。
...
| 文件 | 作用 |
|------|------|
| [__init__.py](file:///d:/workspace/KG-for-structure-file/src/KGRetrieve/__init__.py) | 包入口，导出 KGMemory / KGDBMemory / 工具类 |
| [memory.py](file:///d:/workspace/KG-for-structure-file/src/KGRetrieve/memory.py) | KGMemory 内存索引 + Node/Edge 数据类 |
| [db_backend.py](file:///d:/workspace/KG-for-structure-file/src/KGRetrieve/db_backend.py) | KGDBMemory（Neo4j+ChromaDB 后端）+ 导入脚本 |
| [tools.py](file:///d:/workspace/KG-for-structure-file/src/KGRetrieve/tools.py) | 14 个原子工具 + BaseTool + ToolRegistry（支持双后端） |
| [demo_smoke_test.py](file:///d:/workspace/KG-for-structure-file/src/KGRetrieve/demo_smoke_test.py) | 冒烟测试：14 个工具各调用一遍 |
| [demo_agent_orchestration.py](file:///d:/workspace/KG-for-structure-file/src/KGRetrieve/demo_agent_orchestration.py) | 伪 Agent 多轮编排示例 |
