...
# ====== 模型入口（保持原有） ======
try:
    from KGBuild.model.main import get_encoder
except Exception:
    from KGBuild.model.main import get_encoder  # 兜底 import
...
def main():
    ...
    # BERT 目录在 KGBuild/model/<BERT_DIR_NAME>
    bert_dir_name = emb.get("BERT_DIR_NAME", "bert-base-chinese")
    bert_dir = script_dir / "model" / bert_dir_name
    # 本地目录不存在时，回退到 HuggingFace 在线模型名
    if not bert_dir.exists():
        bert_dir = bert_dir_name  # 直接用 "bert-base-chinese"，让 HF 自动下载
        print(f"⚠️ 本地模型目录不存在，使用 HuggingFace 在线模型：{bert_dir_name}")
